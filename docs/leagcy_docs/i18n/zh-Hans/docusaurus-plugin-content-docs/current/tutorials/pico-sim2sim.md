...
在 `STANDING` 中按键盘 `F` 可进入 `SUSPENDED_ARMS`；再次按 `F` 返回
`STANDING`。此模式让双臂继续跟随，同时通过 PD 位置控制将腿部和腰部保持在进入
模式时测得的关节角度，并忽略策略输出中的非手臂部分。按 Pico 手柄或键盘 `B` 会
切换到普通 `ARMS`，恢复所有关节的策略输出。如果 Pico 追踪不可用，按 `F` 后仿真
仍停留在 `STANDING`；追踪恢复后需再次按 `F`。
