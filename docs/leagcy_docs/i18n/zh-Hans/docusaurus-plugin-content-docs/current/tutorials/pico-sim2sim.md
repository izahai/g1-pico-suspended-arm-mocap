...
`PAUSED` 保持当前参考姿态，恢复后返回之前的活动模式。

按键盘 `1` 选择 `LEFT_SUSPENDED_ARMS`，按 `2` 选择
`RIGHT_SUSPENDED_ARMS`，按 `3` 选择双臂 `SUSPENDED_ARMS`。可从 `IDLE` 或
`STANDING` 进入，并通过数字键直接切换；再次按当前模式的数字键或按 `X` 返回
进入前的模式。只有选中的手臂关节接收策略目标。其他关节通过 PD 位置控制保持
测得的角度；变为非活动状态的手臂会保持切换时测得的角度，腿和腰仍保持原目标
角度。Pico 手柄或键盘 `B` 可将双臂模式切换为普通 `ARMS`，在单臂模式中无效。
如果 Pico 追踪不可用，恢复后需再次按数字键。
