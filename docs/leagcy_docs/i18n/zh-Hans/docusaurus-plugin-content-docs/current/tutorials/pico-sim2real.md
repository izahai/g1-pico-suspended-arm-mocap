...
Teleopit 的交互式终端中，按 **1** 选择 `LEFT_SUSPENDED_ARMS`，按 **2** 选择
`RIGHT_SUSPENDED_ARMS`，按 **3** 选择双臂 `SUSPENDED_ARMS`。
...
三种悬吊手臂模式使用与 `ARMS` 相同的实时手臂参考。只有选中的手臂关节接收策略
目标：左臂为 15–21，右臂为 22–28，双臂为 15–28。其他关节通过 PD 位置控制
保持测得的角度。机器人悬吊时可从 `IDLE` 进入，也可从 `STANDING` 进入。系统最多
等待两秒以获取有效的 Pico 帧；若超时，追踪恢复后需再次按数字键。按 **1**、
**2**、**3** 可直接切换模式。变为非活动状态的手臂会保持切换时测得的角度，腿和腰
仍保持进入时的目标角度。再次按当前模式的数字键或按 G1 遥控器 `X`，返回进入前
的模式。Pico 手柄 `B` 可将双臂悬吊模式切换为普通 `ARMS`；在单臂模式中该键无效。
G1 遥控器 `B` 或 Pico 手柄 `A` 可暂停或恢复追踪。悬吊手臂模式不可录制；
进入任一模式都会丢弃正在录制的 episode。
