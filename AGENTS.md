...
- Pico sim2sim supports a keyboard-driven top-level mode state machine: `STANDING → MOCAP ↔ ARMS` plus `STANDING/IDLE ↔ SUSPENDED_ARMS / LEFT_SUSPENDED_ARMS / RIGHT_SUSPENDED_ARMS`; `X` returns to `STANDING` (or `IDLE` if entered from `IDLE`)
- Default Pico sim2sim keyboard mappings are `Y` → `MOCAP` (or `STANDING` from `IDLE`), `1` → `LEFT_SUSPENDED_ARMS`, `2` → `RIGHT_SUSPENDED_ARMS`, `3` → `SUSPENDED_ARMS`, `A` → pause/resume mocap, `B` → toggle `MOCAP`/`ARMS` or switch `SUSPENDED_ARMS` to `ARMS`, `X` → back to `STANDING` (or `IDLE` if entered from `IDLE`), `Q` → quit
...
- Pico4 sim2real arms the process-isolated reference worker only on an explicit `MOCAP` or suspended arms (keys `1`/`2`/`3`) entry request; idle `STANDING` and `DAMPING` disarm it so cold startup frames do not warm-start GMR before entry
...
- Suspended arm modes are additional Pico modes entered from `STANDING` or directly from `IDLE` with terminal keys `1`/`2`/`3` (sim2real) or keyboard keys `1`/`2`/`3` (sim2sim): `1` for `LEFT_SUSPENDED_ARMS` (left arm joints `[15..21]`), `2` for `RIGHT_SUSPENDED_ARMS` (right arm joints `[22..28]`), and `3` for `SUSPENDED_ARMS` (both arms `[15..28]`). Key `F` is completely removed. Pressing the active mode's key again or remote `X` returns to the mode of origin (`IDLE` if entered from `IDLE`, `STANDING` if entered from `STANDING`). Pressing `1`, `2`, or `3` while in any suspended mode switches directly to that mode on the fly, locking the inactive arm at its current measured angle. Suspended modes use the ARMS reference, capture measured leg/waist (and inactive arm) angles at entry/switch, ignore non-controlled policy outputs, and hold those joints at captured angles with PD. Direct entry from `IDLE` allows teleoperating the arms when the robot is suspended on a hoist with feet off the floor without ever running the standing balance policy. Pico/controller `B` switches `SUSPENDED_ARMS` to regular `ARMS` (ignored in `LEFT_SUSPENDED_ARMS` and `RIGHT_SUSPENDED_ARMS`). Unitree remote `B` and Pico/controller `A` pause/resume them. Sim2real keys `1`/`2`/`3` arm the reference worker, enter after valid frames, and cancel after 2 s without valid tracking; a new key press is then required.
...
- Recording is manual only: terminal `R` starts an episode, `S` saves, `D` discards the active episode, and `Q` shuts down; `STANDING`, `MOCAP`, `ARMS`, and paused mocap are recordable. Suspended arm modes (`SUSPENDED_ARMS`, `LEFT_SUSPENDED_ARMS`, `RIGHT_SUSPENDED_ARMS`) are not recordable and entering them discards an active episode.
