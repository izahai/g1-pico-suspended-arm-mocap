...
- Pico sim2sim supports a keyboard-driven top-level mode state machine: `STANDING → MOCAP ↔ ARMS` plus `STANDING/IDLE ↔ SUSPENDED_ARMS`; `X` returns to `STANDING` (or `IDLE` if entered from `IDLE`)
- Default Pico sim2sim keyboard mappings are `Y` → `MOCAP` (or `STANDING` from `IDLE`), `F` → toggle `STANDING/IDLE`/`SUSPENDED_ARMS`, `A` → pause/resume mocap, `B` → toggle `MOCAP`/`ARMS` or switch `SUSPENDED_ARMS` to `ARMS`, `X` → back to `STANDING` (or `IDLE` if entered from `IDLE`), `Q` → quit
...
- Pico4 sim2real arms the process-isolated reference worker only on an explicit `MOCAP` or `SUSPENDED_ARMS` entry request; idle `STANDING` and `DAMPING` disarm it so cold startup frames do not warm-start GMR before entry
...
- `SUSPENDED_ARMS` is an additional Pico mode entered from `STANDING` or directly from `IDLE` with terminal `F` (sim2real) or keyboard `F` (sim2sim); `F` again or remote `X` returns to the mode of origin (`IDLE` if entered from `IDLE`, `STANDING` if entered from `STANDING`). It uses the ARMS reference, captures measured leg/waist angles at entry, ignores non-arm policy outputs, and holds those joints at the captured angles with PD. Direct entry from `IDLE` allows teleoperating the arms when the robot is suspended on a hoist with feet off the floor without ever running the standing balance policy. Pico/controller `B` switches it to regular `ARMS`, restoring full policy output. Unitree remote `B` and Pico/controller `A` pause/resume it. Sim2real `F` arms the reference worker, enters after valid frames, and cancels after 2 s without valid tracking; a new `F` press is then required.
...
- Recording is manual only: terminal `R` starts an episode, `S` saves, `D` discards the active episode, and `Q` shuts down; `STANDING`, `MOCAP`, `ARMS`, and paused mocap are recordable. `SUSPENDED_ARMS` is not recordable and entering it discards an active episode.
