...
- Pico sim2sim supports a keyboard-driven top-level mode state machine: `STANDING → MOCAP ↔ ARMS` plus `STANDING ↔ SUSPENDED_ARMS`; `X` returns to `STANDING`
- Default Pico sim2sim keyboard mappings are `Y` → `MOCAP`, `F` → toggle `STANDING`/`SUSPENDED_ARMS`, `A` → pause/resume mocap, `B` → toggle `MOCAP`/`ARMS` or switch `SUSPENDED_ARMS` to `ARMS`, `X` → back to `STANDING`, `Q` → quit
...
- `SUSPENDED_ARMS` is an additional Pico mode entered only from `STANDING` with terminal `F` (sim2real) or keyboard `F` (sim2sim); `F` again returns to `STANDING`. It uses the ARMS reference, captures measured leg/waist angles at entry, ignores non-arm policy outputs, and holds those joints at the captured angles with PD. Pico/controller `B` switches it to regular `ARMS`, restoring full policy output. Unitree remote `B` and Pico/controller `A` pause/resume it. Sim2real `F` arms the reference worker, enters after valid frames, and cancels after 2 s without valid tracking; a new `F` press is then required.
